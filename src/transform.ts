...
class ModifyJson extends Transform {
  _transform(
    chunk: Person,
...
class changeObjectFormat extends Transform {
  _transform(
    chunk: any,
    encoding: BufferEncoding,
    callback: TransformCallback
...
const readStream = createReadStream("hugo.json");
const writeStream = createWriteStream("copy-hugo.json");
...
const dataReadStream = createReadStream("BigData.json");
const dataWriteStream = createWriteStream("BigData_copy.json");
const dataParser = JSONStream.parse("*");
const dataStringify = JSONStream.stringify();
const resume = createWriteStream("resume.txt");
...
dataParser.on("data", (chunk: Buffer) => {
